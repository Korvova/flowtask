...
window.TemplateManager = {
    ...
    /**
     * Проверить существование хранилища для шаблонов
    ...
    ensureTemplateStorage: async function() {
        return new Promise((resolve, reject) => {
            ...
            console.log('🔍 Проверяем существование хранилища tflow_tmpl через entity.get...');

            // Просто проверяем что хранилище существует
            BX24.callMethod('entity.get', {
                ENTITY: 'tflow_tmpl'
            }, (result) => {
                if (result.error()) {
                    console.error('❌ Хранилище tflow_tmpl не найдено!');
                    console.error('   Необходимо создать хранилище вручную через install.php');
                    console.error('   Или обратиться к администратору');
                    reject(new Error('Хранилище tflow_tmpl не существует. Переустановите приложение.'));
                } else {
                    console.log('✅ Хранилище tflow_tmpl существует');
